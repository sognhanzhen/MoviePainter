...
export type AdminDashboardSummary = {
  activeUsers: number;
  curatedPosters: number;
  pendingReviews: number;
  recommendations: number;
  tags: number;
};

export type AdminPosterItem = {
...
export function getPosterById(id: string | null) {
  if (!id) {
    return null;
  }

  return posterRecords.find((poster) => poster.id === id) ?? null;
}
